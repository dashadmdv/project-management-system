...
class Database:
    ...
    def add_project(self, name: str):
        try:
            ...
            """
            self.execute(
                sql, (name, datetime.datetime.today().strftime('%Y-%m-%d')))
    ...
    def add_contract(self, name: str):
        try:
            ...
            """
            self.execute(
                sql, (name, datetime.datetime.today().strftime('%Y-%m-%d'), 'draft'))
    ...
    def add_contract_to_project(self, contract_id, project_id):
        self.execute("SELECT status FROM contract WHERE id = %s;",
                     (contract_id,))
        status = self.fetchone()[0]
        self.execute(
            "SELECT project_id FROM contract WHERE id = %s;", (contract_id,))
        has_parent_project = isinstance(self.fetchone()[0], int)
